...
export class Game extends THREE.Group {
  constructor(userRig, relPos = new THREE.Vector3(0, 0, 0), relRot = new THREE.Vector3(0, 0, 0), ducksAmt = 5, duckBaseSpeed = 5, projRadius = 3, projSpeed = 1, projRateOfFire = 3) {
    ...
    let buttons = [
      new MyGuiVRButton('Window height', this.WINDOW_HEIGHT, 0, 1.95, false, (x) => {
        this.WINDOW_HEIGHT = x;
        this.WINDOW_HEIGHT_END = this.WINDOW_HEIGHT_BEGIN + this.WINDOW_HEIGHT;
        this.buildBackWall();
      }),
      new MyGuiVRButton('Amount of ducks', this.DUCKS_AMT, 1, 10, true, (x) => {
          this.duckRig.ducksAmt = x;
          this.duckRig.reset();
      }),
      new MyGuiVRButton('Duck speed', this.DUCK_BASE_SPEED, 1, 10, true, (x) => {
        this.duckRig.duckBaseSpeed = x / 200;
        this.duckRig.reset();
      }),
      new MyGuiVRButton('Projectile radius', this.PROJ_RADIUS, 1, 5, true, (x) => {
          this.PROJ_RADIUS = x**2 / 100;
      }),
      new MyGuiVRButton('Projectile speed', this.PROJ_SPEED, 1, 5, true, (x) => {
          this.PROJ_SPEED = 3 * x / 4;
      }),
      new MyGuiVRButton('Rate of fire', this.PROJ_RATE_OF_FIRE, 1, 5, true, (x) => {
    ...
    this.playButton = new MyGuiVRButton('PLAY', 0, 0, 1, true, (x) => {
      let controller = this.userRig.getController(0);
  ...
  buildGame() {
    ...
    fontLoader.load('extern/fonts/helvetiker_bold.typeface.json', (font) => {
        let geometry = new THREE.TextBufferGeometry('Tip: Click the brown door\nnext to this text to enter/exit\nthe Duckshooter VR game room.', {
            font: font,
            size: 0.06,
            height: 0.001,
            curveSegments: 3,
      ...
      geometry = new THREE.TextBufferGeometry('Welcome to Duckshooter VR!\nThere are 2 menus - one is on your left to start/stop the game,\nthe other one is on your right to change the game settings. The\ngoal of this game is to shoot all of the moving ducks (use the\ntrigger on your right controller to shoot). The score will be\ndetermined by the time it takes you to shoot all of the ducks.\nRemember that you can always use the door behind you to leave.', {
          font: font,
          size: 0.06,
          height: 0.001,
          curveSegments: 3,
  ...
  showScore() {
    ...
    let fontLoader = new THREE.FontLoader();
    fontLoader.load('extern/fonts/helvetiker_bold.typeface.json', (font) => {
      let geometry = new THREE.TextBufferGeometry('Score: ' + this.score, {
          font: font,
          size: 0.06,
          height: 0.001,
          curveSegments: 3,
